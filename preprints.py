...
def parse(fp):
    while True:
        try:
            ...
            p.authors = fp.skip()
            p.authors = re.sub(r'((\w+\.[~\-\s]+)*(Frolov|Pogosian|Zhao|Berndsen|Gooding|Zylberberg|Hojjati|Guo|Talbot|Wang|Brush|Galvez~Ghersi|Gálvez~Ghersi|Plahn|Zucca))', r'<B>\1</B>', p.authors)
